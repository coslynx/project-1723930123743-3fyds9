...
const GoalForm: React.FC = () => {
  ...
  const createGoalMutation = useMutation(
    (goalData: GoalFormData) => axios.post('/api/goals', goalData),
    {
      onSuccess: () => {
  ...
  const updateGoalMutation = useMutation(
    (goalData: GoalFormData & { id: string }) => axios.put(`/api/goals/${goalData.id}`, goalData),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['goals', session?.user?.id]);
  ...
  const deleteGoalMutation = useMutation(
    (goalId: string) => axios.delete(`/api/goals/${goalId}`),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['goals', session?.user?.id]);
